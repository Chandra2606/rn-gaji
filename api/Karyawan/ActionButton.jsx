...
const ActionButton = ({kdkaryawan}) => {
  ...
  const actionDeleteData = async () => {
    ...
    Alert.alert(
      'konfirmasi',
      'Apakah anda yakin ingin menghapus data karyawan atau guru ini?',
      [
        {
          text: 'Batal',
        ...
        },
        {
          ...
          onPress: () => {
            ...
            fetch(`${apiUrl}karyawan/${kdkaryawan}`, {
              method: 'DELETE',
              headers: {
              ...
              .then(response => {
                if (response.status === 200) {
                  Alert.alert('', 'Data karyawan Atau Guru berhasil dihapus!', [
                    {
                      text: ' ok',
                      onPress: () =>
  ...
  const showFormEdit = () => {
    navigation.navigate('FormEdit', {kdkaryawan: kdkaryawan});
  };
  return (
    <View
      style={{
