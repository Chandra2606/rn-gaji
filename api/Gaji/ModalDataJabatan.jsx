...
export default function ModalDataJabatan({
  isVisible,
  onClose,
  onJabatanSelected,
}) {
  ...
  const fetchDataJabatan = async (pageNumber = 1, searchQuery = search) => {
    if (pageNumber === 1) {
    ...
    try {
      ...
      setDataJabatan(
        pageNumber === 1 ? json.data : [...dataJabatan, ...json.data],
      );
    } catch (error) {
  ...
  const pilihDataJabatan = item => {
    onJabatanSelected(item.kdjabatan, item.namajabatan, item.gapok, item.tunjangan);
  };
  // Render Item untuk FlatList
  const renderItemJabatan = ({item}) => (
    <TouchableOpacity
      onPress={() => pilihDataJabatan(item)}
      style={{padding: 10, borderBottomWidth: 1, borderBottomColor: '#ccc'}}>
      <Text>{item.kdjabatan}</Text>
      <Text>{item.namajabatan}</Text>
      <Text>{item.gapok}</Text>
      <Text>{item.tunjangan}</Text>
    </TouchableOpacity>
  );
  return (
    <Overlay
