...
export default function DataGaji() {
  ...
  //memanggil api
  const fetchdataServis = async (pageNumber = 1, searchQuery = search) => {
    let token = await AsyncStorage.getItem('userToken');
    setLoading(true);
    setError('');
    try {
      const response = await fetch(
        `${apiUrl}gaji?page=${pageNumber}&search=${searchQuery}`,
        {
          headers: {
      ...
      setDataServis(
        pageNumber === 1 ? json.data : [...dataServis, ...json.data],
      );
    } catch (error) {
  ...
  const onRefresh = () => {
    setRefreshing(true);
    fetchdataServis(1, search).finally(() => setRefreshing(false));
  };
  ...
  const rederItemMahasiswa = ({item}) => {
    const showDetailData = () => {
      navigation.navigate('DetailData', {kdgaji: item.kdgaji});
    };
    return (
      <TouchableOpacity style={styles.item} onPress={showDetailData}>
        <Text style={styles.title}>
          {item.nama_lengkap} - {item.karyawankd}
        </Text>
        <Text style={styles.kdgaji}>
          {item.kdgaji} - {item.total}
        </Text>
      </TouchableOpacity>
    );
  };
    ...
    <View style={styles.container}>
      <View>
        <TextInput
          ...
          placeholder="Cari Berdasarkan Kode Gaji Atau Nama Karyawan"
          value={search}
      ...
      <FlatList
        data={dataServis}
        renderItem={rederItemMahasiswa}
        keyExtractor={item => item.kdgaji}
        extraData={loading || error}
        onEndReached={() => {
          if (!loading && page < lastPage) {
            fetchdataServis(page + 1);
          }
        }}
