...
export default function Data() {
  ...
  const rederItemMobil = ({item}) => {
    const showDetailData = () => {
      navigation.navigate('DetailMobil', {kdmobil: item.kdmobil});
    };
    return (
      <TouchableOpacity style={styles.item} onPress={showDetailData}>
        <Text style={styles.title}>{item.namamobil}</Text>
        <Text style={styles.nim}>{item.kdmobil}</Text>
      </TouchableOpacity>
    );
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        keyExtractor={item => item.kdmobil}
